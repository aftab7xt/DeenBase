...
function activateSearchMode() {
    ...
    // Always unhide header search when Search tab is clicked
...
function showHistory() {
    const container = document.getElementById('searchHistory'); // Fixed ID
    if(!container) return;
...
function showFavorites() {
    ...
    if(favorites.length === 0) {
        display.innerHTML = `<div style="text-align:center; margin-top:50px; color:var(--secondary-text);"><h3>No Favorites Yet</h3><p>Tap the heart icon on any Hadith to save it here.</p><button class="back-home-btn" onclick="goHome()">← Start Reading</button></div>`;
        return;
    }
